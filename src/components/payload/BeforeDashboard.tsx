...
export const BeforeDashboard = () => {
  ...
  const hasReadPermission = (collectionSlug: string): boolean => {
    ...
    if (user.role === 'editor') {
      const adminOnly = ['users', 'roles', 'siteSettings', 'payments'];
      return !adminOnly.includes(collectionSlug);
    }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {[
          { label: 'Treks', count: stats.treks, color: '#1a2e1f', bg: '#eef3ef', slug: 'treks' },
          { label: 'Departures', count: stats.departures, color: '#c8922a', bg: '#fbf7ee', slug: 'departures' },
          { label: 'Active Bookings', count: stats.bookings, color: '#059669', bg: '#ecfdf5', slug: 'bookings' },
          { label: 'Inquiries', count: stats.inquiries, color: '#3b82f6', bg: '#eff6ff', slug: 'inquiries' },
          { label: 'Blog Posts', count: stats.blogs, color: '#7c3aed', bg: '#f5f3ff', slug: 'blogPosts' },
          { label: 'Payments Recv.', count: stats.payments, color: '#e11d48', bg: '#fff1f2', slug: 'payments' },
        ].filter(item => hasReadPermission(item.slug)).map((item, idx) => (
          <div key={idx} style={{
            background: item.bg,
            borderRadius: '12px',
            padding: '16px',
      ...
      {/* Section Header: Core Content & Operations */}
      {(hasReadPermission('blogPosts') || hasReadPermission('treks') || hasReadPermission('departures') || hasReadPermission('bookings')) && (
        <>
          <h2 style={{
            fontSize: '18px',
            fontWeight: 800,
      ...
      {/* Section Header: Website Customization & Queries */}
      {(hasReadPermission('siteSettings') || hasReadPermission('inquiries') || hasReadPermission('payments') || hasReadPermission('regions') || hasReadPermission('teamMembers') || hasReadPermission('testimonials') || hasReadPermission('gallery') || hasReadPermission('faqs') || hasReadPermission('homepageSettings') || hasReadPermission('navbarSettings') || hasReadPermission('footerSettings') || hasReadPermission('companyPages') || hasReadPermission('contactPages') || hasReadPermission('blogSettings')) && (
        <>
          <h2 style={{
            fontSize: '18px',
            fontWeight: 800,
          ...
          <div style={{
            ...
            {hasReadPermission('siteSettings') && (
              <Link 
            ...
            {/* Card: Homepage Settings */}
            {hasReadPermission('homepageSettings') && (
              <Link 
                href="/admin/collections/homepageSettings"
                style={{
                  background: '#ffffff',
                  border: '1px solid #e2e8f0',
                  borderRadius: '12px',
              ...
              >
                <div style={{ fontSize: '24px', marginBottom: '8px' }}>🏠</div>
                <h3 style={{ margin: '0 0 4px 0', fontSize: '15px', fontWeight: 700, color: '#1a2e1f' }}>Homepage Settings</h3>
                <p style={{ margin: 0, fontSize: '12px', color: '#64748b', lineHeight: '1.4' }}>
                  Customize the split-layout sections, highlights, and custom badge assets.
                </p>
                <div style={{ marginTop: '12px', fontSize: '11px', fontWeight: 700, color: '#c8922a' }}>
            ...
            {/* Card: Testimonials */}
            {hasReadPermission('testimonials') && (
              <Link 
